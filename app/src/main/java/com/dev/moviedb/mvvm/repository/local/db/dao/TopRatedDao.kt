 ...
 */
@Dao
abstract class TopRatedDao : IDao<Movie>{
    ...
    @Query("UPDATE FROM top_rated_movie WHERE id = :elem.identifier")
    abstract override fun update(elem: Movie, onComplete: ITaskListener)

    @Query("UPDATE FROM top_rated_movie")
    abstract override fun updateAll(vararg elements: Movie, onComplete: ITaskListener)

    @Query("DELETE FROM top_rated_movie WHERE id = :elem.identifier")
